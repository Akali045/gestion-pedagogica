...
function renderTeamsSection(group) {
    const container = document.getElementById('teamsSectionContainer');
    if (!container) return;

    const html = `
        <div class="teams-section group-info" style="margin-top: 2rem;">
...
function executeTeamGeneration(students, distributionSizes, criteria) {
    ...
    const groupForOpt = getCurrentGroup();
    if (groupForOpt) {
    ...
    if (criteria === 'random') {
        ...
        // Snake draft by leadership (unique peer reach + nomination intensity)
        const uniqueChoosers = {};
        const totalMentions = {};
        studentsPool.forEach(s => {
            uniqueChoosers[s.id] = new Set();
            totalMentions[s.id] = 0;
        });
                ...
                ['academic', 'social', 'complementary'].forEach(type => {
                    if (s.sociogram[type]) {
                        s.sociogram[type].forEach(targetId => {
                            if (uniqueChoosers[targetId]) {
                                uniqueChoosers[targetId].add(s.id);
                                totalMentions[targetId]++;
                            }
                        });
                    }
                });
        ...
        // Sort primarily by unique peer reach, secondarily by total mentions
        studentsPool.sort((a, b) => {
            const diffU = uniqueChoosers[b.id].size - uniqueChoosers[a.id].size;
            if (diffU !== 0) return diffU;
            return totalMentions[b.id] - totalMentions[a.id];
        });
...
function renderGeneratedTeams() {
    ...
    currentTeams.forEach(team => {
        ...
        html += '<div class="team-card" data-team-id="' + escapeHTML(team.id) + '" style="' + borderStyle + '">';
        
        let headerText = '<h4>' + escapeHTML(team.name) + ' (' + team.members.length + ')';
        if (hasConflict) {
        ...
        team.members.forEach(studentId => {
            ...
            if (student) {
                const color = student.color || '#64748b';
                const name = student.preferredName || student.fullName || student.name || 'Sin Nombre';
                ...
                html += '<div class="team-member-item" data-student-id="' + escapeHTML(student.id) + '" style="' + itemStyle + '">';
                html += '<div class="member-color-dot" style="background-color: ' + escapeHTML(color) + '"></div>';
                html += '<span style="' + (teamConflictIds.has(studentId) ? 'font-weight: 600;' : '') + '">' + escapeHTML(name) + '</span>';
                if (channelText) html += '<span class="' + channelClass + '">' + escapeHTML(channelText) + '</span>';
    ...
    group.teamSets.slice().reverse().forEach(set => {
        html += `
            <div class="saved-team-card">
                <div>
                    <strong>${escapeHTML(set.name)}</strong> <span style="color: var(--text-secondary); font-size: 0.85rem;">- ${escapeHTML(set.date)}</span><br>
                    <span style="font-size: 0.85rem;">Criterio: ${escapeHTML(set.criteria)} | Tamaño sugerido: ${escapeHTML(set.teamSize)} | ${set.teams.length} equipos</span>
                </div>
...
function generateSVGAndDownload(teamsToExport, overrideName = null) {
    ...
    const group = getCurrentGroup();
    const titleName = overrideName || (group ? group.name : 'Equipos');
