...
function openStudentModal(studentId = null) {
    ...
    if (studentId) {
        ...
        const group = getCurrentGroup();
        const student = group ? group.students.find(s => s.id === studentId) : null;
        if (student) {
...
function openImportStudentsModal() {
    ...
    const pasteInput = document.getElementById('importRawText') || document.getElementById('importPasteTextarea');
    if (pasteInput) pasteInput.value = '';

    const fileInput = document.getElementById('importFileInput') || document.getElementById('importCsvFileInput');
    if (fileInput) fileInput.value = '';

    const previewContainer = document.getElementById('importPreviewContainer');
    if (previewContainer) previewContainer.style.display = 'none';

    const previewTbody = document.getElementById('importPreviewBody') || document.getElementById('importPreviewTbody');
    if (previewTbody) previewTbody.innerHTML = '';

    const confirmBtn = document.getElementById('confirmImportStudentsBtn') || document.getElementById('confirmImportBtn');
...
function switchImportTab(tab) {
    ...
    if (tab === 'paste') {
        if (pasteBtn) { pasteBtn.classList.add('active', 'btn-primary'); pasteBtn.classList.remove('btn-secondary'); }
        if (fileBtn) { fileBtn.classList.remove('active', 'btn-primary'); fileBtn.classList.add('btn-secondary'); }
        if (pasteSection) pasteSection.style.display = 'block';
        if (fileSection) fileSection.style.display = 'none';
    } else {
        if (pasteBtn) { pasteBtn.classList.remove('active', 'btn-primary'); pasteBtn.classList.add('btn-secondary'); }
        if (fileBtn) { fileBtn.classList.add('active', 'btn-primary'); fileBtn.classList.remove('btn-secondary'); }
...
function renderImportPreview(studentsList) {
    ...
    const tbody = document.getElementById('importPreviewBody') || document.getElementById('importPreviewTbody');
    const badge = document.getElementById('importParsedCount') || document.getElementById('importCountBadge');
    const confirmBtn = document.getElementById('confirmImportStudentsBtn') || document.getElementById('confirmImportBtn');
    ...
    if (badge) badge.textContent = studentsList.length;
    if (tbody) {
        tbody.innerHTML = studentsList.map((st, idx) => `
            <tr>
                <td style="font-weight: 600; text-align: center; padding: 4px;">
                    <input type="number" class="import-preview-num" data-idx="${idx}" value="${escapeHTML(st.listNumber)}" min="1" style="width: 50px; padding: 2px 4px; font-size: 0.85rem; border: 1px solid var(--border); border-radius: 4px; background: var(--bg-primary); color: var(--text-primary); text-align: center;">
                </td>
                ...
                <td style="padding: 4px;">
                    <input type="text" class="import-preview-shortname" data-idx="${idx}" value="${escapeHTML(st.preferredName || '')}" placeholder="Opcional" style="width: 100%; padding: 2px 6px; font-size: 0.85rem; border: 1px solid var(--border); border-radius: 4px; background: var(--bg-primary); color: var(--text-primary);">
                </td>
        ...
        tbody.querySelectorAll('.import-preview-shortname').forEach(input => {
            input.addEventListener('change', (e) => {
                const idx = parseInt(e.target.dataset.idx, 10);
                const val = e.target.value.trim();
                if (parsedImportStudentsList[idx]) {
                    parsedImportStudentsList[idx].preferredName = val;
                }
            });
        });
    }
