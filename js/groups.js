...
function showGroupDetail(groupId) {
    ...
    // Ensure completedPDAs and observations exist for backward compatibility
                ...
                <div class="pda-activities-section" style="margin-top: 1.5rem;">
                    ...
                    <div id="pdaActivitiesContainer">
                        ...
                        ` : group.pdaActivities.map(act => {
                            ...
                            return `
                                <div class="activity-card" style="margin-bottom: 0.5rem; padding: 0.5rem; background: var(--bg-secondary); border-radius: 4px;">
                                    <div class="activity-info">
                                        <div class="activity-title-row" style="display: flex; justify-content: space-between; align-items: center;">
                                            <span class="activity-name" style="font-weight: 600;">${act.name}</span>
                                            <span class="activity-badge ${statusInfo.class}" style="font-size: 0.75rem; padding: 0.2rem 0.5rem; border-radius: 4px;">${statusInfo.label}</span>
                                        </div>
                                        ${act.description ? `<p class="activity-description" style="font-size: 0.9rem; margin: 0.25rem 0 0;">${act.description}</p>` : ''}
                                    </div>
                                    <div class="activity-actions" style="display: flex; gap: 0.25rem; justify-content: flex-end; margin-top: 0.5rem;">
                                        <button class="btn btn-small btn-secondary edit-activity-btn" data-act-id="${act.id}" title="Editar" style="padding: 0.25rem 0.5rem; font-size: 0.85rem; width: auto;">✏️</button>
                                        <button class="btn btn-small btn-danger delete-activity-btn" data-act-id="${act.id}" title="Eliminar" style="padding: 0.25rem 0.5rem; font-size: 0.85rem; width: auto;">🗑️</button>
                                    </div>
                                </div>
                            `;
                        }).join('')}
    ...
    pdaHistorySection.className = 'pda-history-section group-info';
            ...
            <div class="pda-history-card" data-idx="${item.idx}">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem;">
                    <div style="flex-grow: 1;">
                        ...
                        <p style="margin: 0.25rem 0 0.5rem 0; font-size: 0.95rem; color: var(--text-secondary);">${item.pda}</p>
                        <div class="dates">
                            ...
                            <div class="pda-history-activities" style="margin-top: 0.75rem; border-top: 1px dashed var(--border); padding-top: 0.5rem;">
                                ...
                                <div style="display: flex; flex-direction: column; gap: 0.35rem;">
                                    ${item.activities.map(act => {
                                        const statusInfo = ACTIVITY_STATUSES[act.status] || { label: 'Desconocido', class: '' };
                                        return `
                                            <div style="display: flex; flex-direction: column; background: var(--bg-secondary); padding: 0.4rem; border-radius: 4px; border: 1px solid var(--border); font-size: 0.85rem;">
                                                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.25rem;">
                                                    <span style="font-weight: 600; color: var(--text-primary);">${act.name}</span>
                                                    <span class="activity-badge ${statusInfo.class}" style="font-size: 0.7rem; padding: 0.1rem 0.3rem; border-radius: 4px;">${statusInfo.label}</span>
                                                </div>
                                                ${act.description ? `<p style="margin: 0.25rem 0 0 0; font-size: 0.8rem; color: var(--text-secondary); white-space: pre-wrap;">${act.description}</p>` : ''}
                                            </div>
                                        `;
                                    }).join('')}
                                </div>
    ...
    // Add event listeners for edit and delete completed PDA buttons
    pdaHistorySection.querySelectorAll('.edit-completed-pda-btn').forEach(btn => {
    ...
    pdaHistorySection.querySelectorAll('.delete-completed-pda-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const idx = parseInt(btn.dataset.idx);
...
function renderGroupObservations(group) {
    ...
    let html = `
        <div class="group-info" style="border-left: 5px solid var(--accent); margin-top: 2rem;">
            ...
            <div style="margin-bottom: 1.5rem;">
                <textarea id="newGroupObservationText" placeholder="Escribe un registro de lo ocurrido en la clase de hoy..." style="margin-bottom: 0.75rem;"></textarea>
    ...
    if (sorted.length === 0) {
        ...
        sorted.forEach(obs => {
            ...
            if (mentionedIds.length >= 2) {
                ...
                pairHtml += `
                    <div class="tension-override-wrapper" style="margin-top: 0.75rem;">
                        <div class="tension-override-header" data-obs-id="${obs.id}" style="cursor: pointer; display: flex; justify-content: space-between; align-items: center; background: var(--bg-tertiary); padding: 0.5rem 0.75rem; border-radius: 8px; border: 1px solid var(--border); font-size: 0.8rem; font-weight: 600; color: var(--text-secondary); transition: all 0.2s ease;">
                        ...
                        <div class="tension-override-content" id="tensionContent_${obs.id}" style="display: none; background: var(--bg-tertiary); padding: 0.75rem; border-radius: 0 0 8px 8px; border: 1px solid var(--border); border-top: none; margin-top: -1px;">
                    ...
                    for (let j = i + 1; j < mentionedIds.length; j++) {
                        ...
                        pairHtml += `
                            <div class="pair-relationship-toggle" style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; background: var(--bg-primary); padding: 0.4rem 0.6rem; border-radius: 8px; border: 1px solid var(--border); width: 100%;">
                                <span style="font-weight: 600; color: var(--text-primary); font-size: 0.8rem; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">${labelA} y ${labelB}:</span>
                                <div class="relationship-segment-control">
                                    <label class="${isConflictChecked ? 'selected-tension' : ''}">
                                        <input type="radio" name="rel_${obs.id}_${pairId}" class="relation-radio tension-radio" data-obs-id="${obs.id}" data-pair-id="${pairId}" value="conflict" ${isConflictChecked ? 'checked' : ''}>
                                        ⚡ Tensión
                                    </label>
                                    <label class="${isSupportChecked ? 'selected-support' : ''}">
                                        <input type="radio" name="rel_${obs.id}_${pairId}" class="relation-radio support-radio" data-obs-id="${obs.id}" data-pair-id="${pairId}" value="support" ${isSupportChecked ? 'checked' : ''}>
                                        🤝 Apoyo
                                    </label>
                                    <label class="${isNeutralChecked ? 'selected-neutral' : ''}">
                                        <input type="radio" name="rel_${obs.id}_${pairId}" class="relation-radio neutral-radio" data-obs-id="${obs.id}" data-pair-id="${pairId}" value="neutral" ${isNeutralChecked ? 'checked' : ''}>
                                        No aplica
                                    </label>
                                </div>
                            </div>
            ...
            html += `
                <div class="observation-entry" data-obs-id="${obs.id}" style="border-bottom: 1px solid var(--border); padding: 1rem 0;">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem;">
                        <div style="flex: 1;">
                            <strong style="color: var(--text-tertiary); font-size: 0.85rem;">${obs.date}</strong>
                            <p style="margin-top: 0.25rem; white-space: pre-wrap; color: var(--text-primary);">${obs.text}</p>
                            ${pairHtml}
                        </div>
                        <div class="observation-actions" style="display: flex; gap: 0.5rem; flex-shrink: 0;">
                            <button class="btn btn-small btn-secondary edit-group-obs-btn" data-obs-id="${obs.id}" style="width: auto; padding: 0.25rem 0.5rem;">✏️</button>
                            <button class="btn btn-small btn-danger delete-group-obs-btn" data-obs-id="${obs.id}" style="width: auto; padding: 0.25rem 0.5rem;">🗑️</button>
                        </div>
                    </div>
                </div>
            `;
