...
function renderGroupSociogram(group) {
    ...
    container.innerHTML = `
        <div id="sociogramGroupSection" class="group-info" style="margin-top: 2rem;">
            ...
            <div id="sociogramGroupContent" style="display: none; margin-top: 1.5rem;">
                <div id="sociogramFullscreenWrapper" style="display: flex; flex-direction: column; background: var(--bg-primary); position: relative; width: 100%;">
                    
                    <!-- Floating Restore Button (Visible only when user hides HUD in Fullscreen) -->
                    <button id="sociogramRestoreHudBtn" type="button" title="Mostrar barra de controles">
                        ⚙️ Mostrar Controles
                    </button>

                    <!-- Unified Floating HUD Panel -->
                    <div id="sociogramHudPanel" class="sociogram-hud-panel">
                        <!-- Main Bar: Connection Filters + Quick Actions -->
                        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.5rem;">
                            <!-- Connection Filter Badges -->
                            <div style="display: flex; gap: 0.4rem; flex-wrap: wrap; align-items: center;">
                                <label class="sociogram-filter-chip">
                                    <input type="checkbox" id="filterAcademic" checked>
                                    <span style="width:16px;height:3px;background:${SOCIOGRAM_EDGE_COLORS.academic};display:inline-block;border-radius:2px;"></span> Académica
                                </label>
                                <label class="sociogram-filter-chip">
                                    <input type="checkbox" id="filterSocial" checked>
                                    <span style="width:16px;height:3px;background:${SOCIOGRAM_EDGE_COLORS.social};display:inline-block;border-radius:2px;"></span> Social
                                </label>
                                <label class="sociogram-filter-chip">
                                    <input type="checkbox" id="filterComplementary" checked>
                                    <span style="width:16px;height:3px;background:${SOCIOGRAM_EDGE_COLORS.complementary};display:inline-block;border-radius:2px;"></span> Complementaria
                                </label>
                                <label class="sociogram-filter-chip">
                                    <input type="checkbox" id="filterSupports" checked>
                                    <span style="width:16px;height:3px;background:#eab308;display:inline-block;border-radius:2px;"></span> Apoyos
                                </label>
                                <label class="sociogram-filter-chip">
                                    <input type="checkbox" id="filterTensions" checked>
                                    <span style="width:16px;height:3px;background:#ef4444;display:inline-block;border-radius:2px;"></span> Tensiones
                                </label>
                                <label class="sociogram-filter-chip" style="border-color: rgba(239, 68, 68, 0.35);">
                                    <input type="checkbox" id="filterExclusion">
                                    <span style="display:inline-block;padding:1px 5px;border-radius:4px;background:var(--danger);color:white;font-size:0.7rem;font-weight:bold;">⚠️ Exclusión</span>
                                </label>
                            </div>

                            <!-- Actions & Fullscreen Controls -->
                            <div style="display: flex; align-items: center; gap: 0.4rem; margin-left: auto; flex-wrap: wrap;">
                                <button type="button" id="toggleSociogramDrawerBtn" class="btn btn-small btn-secondary" style="font-size: 0.8rem; padding: 0.3rem 0.65rem;" title="Filtros por canal, interés y flechas">
                                    ⚙️ Opciones <span id="sociogramDrawerChevron">▼</span>
                                </button>
                                <button type="button" id="hideHudBtn" class="btn btn-small btn-secondary" style="display: none; font-size: 0.8rem; padding: 0.3rem 0.65rem;" title="Ocultar controles para pantalla completa limpia">
                                    👁️ Ocultar barra
                                </button>
                                <button id="exitFullscreenBtn" class="btn btn-small btn-danger" style="display: none; align-items: center; gap: 0.35rem; font-size: 0.8rem; padding: 0.3rem 0.7rem; border-radius: 6px;" title="Salir de pantalla completa">
                                    ✕ Salir
                                </button>
                            </div>
                        </div>

                        <!-- Collapsible Drawer (Canal, Interés, Flechas y Leyenda) -->
                        <div id="sociogramDrawerContent" class="sociogram-drawer-row" style="display: none;">
                            <div style="display: flex; align-items: center; gap: 0.4rem;">
                                <span style="font-weight: 600; color: var(--text-secondary);">Canal:</span>
                                <select id="crossLearningChannel" style="padding: 0.2rem 0.5rem; font-size: 0.8rem; border-radius: 4px; border: 1px solid var(--border); background: var(--bg-primary); color: var(--text-primary);">
                            ...
                            <div style="display: flex; align-items: center; gap: 0.4rem;">
                                <span style="font-weight: 600; color: var(--text-secondary);">Interés:</span>
                                <select id="crossInterest" style="padding: 0.2rem 0.5rem; font-size: 0.8rem; border-radius: 4px; border: 1px solid var(--border); background: var(--bg-primary); color: var(--text-primary); max-width: 140px;">
                                    <option value="">Todos</option>
                                    ${uniqueInterests.map(interest => `<option value="${escapeHTML(interest)}">${escapeHTML(interest)}</option>`).join('')}
                                </select>
                            </div>
                            <div style="display: flex; align-items: center; gap: 0.4rem;">
                                <span style="font-weight: 600; color: var(--text-secondary);">Flechas:</span>
                                <select id="arrowModeSelect" style="padding: 0.2rem 0.5rem; font-size: 0.8rem; border-radius: 4px; border: 1px solid var(--border); background: var(--bg-primary); color: var(--text-primary); cursor: pointer;">
                                    <option value="dual">⇄ Dobles</option>
                                    <option value="bidirectional">↔️ Bidireccionales</option>
                                </select>
                            </div>
                            <div style="display: flex; align-items: center; gap: 0.6rem; margin-left: auto; flex-wrap: wrap;">
                                <span style="color: var(--text-tertiary); font-size: 0.78rem; font-weight: 600;">Bordes:</span>
                                ${Object.entries(CHANNEL_COLORS).filter(([k]) => k !== '').map(([channel, color]) =>
        `<span style="display: flex; align-items: center; gap: 0.25rem; font-size: 0.78rem; color: var(--text-secondary);">
                                        <span style="width: 10px; height: 10px; border-radius: 50%; border: 2px solid ${color}; display: inline-block;"></span> ${escapeHTML(channel)}
                                    </span>`
    ).join('')}
                            </div>
                        </div>
                    ...
                    <div id="sociogramGraphContainer" style="background: var(--bg-tertiary); border-radius: 12px; position: relative; overflow: hidden; min-height: 520px; width: 100%; flex-grow: 1;"></div>
                </div>
    ...
    const drawerBtn = document.getElementById('toggleSociogramDrawerBtn');
    if (drawerBtn) {
        drawerBtn.addEventListener('click', () => {
            const drawer = document.getElementById('sociogramDrawerContent');
            const chevron = document.getElementById('sociogramDrawerChevron');
            if (drawer) {
    ...
    // Fullscreen HUD Hide/Restore
    const hideHudBtn = document.getElementById('hideHudBtn');
    const restoreHudBtn = document.getElementById('sociogramRestoreHudBtn');
    const hudPanel = document.getElementById('sociogramHudPanel');

    if (hideHudBtn && hudPanel && restoreHudBtn) {
        hideHudBtn.addEventListener('click', () => {
            hudPanel.classList.add('hud-minimized');
            restoreHudBtn.style.display = 'inline-flex';
        });

        restoreHudBtn.addEventListener('click', () => {
            hudPanel.classList.remove('hud-minimized');
            restoreHudBtn.style.display = 'none';
        });
    }
...
function renderSociogramGraph(group) {
    ...
    const pairRelationshipScores = new Map();
    ...
    for (let i = 0; i < students.length; i++) {
        for (let j = i + 1; j < students.length; j++) {
            ...
            pairRelationshipScores.set(pairKey, score);
            if (score > 0) {
    ...
    const simulation = d3.forceSimulation(nodes)
        .force('link', d3.forceLink(links).id(d => d.id)
            .distance(d => {
                if (d.type === 'tension') {
                    // Fuerza física de separación / repulsión para alumnos en tensión
                    const u = d.source.id || d.source;
                    const v = d.target.id || d.target;
                    const pairKey = u < v ? `${u}_${v}` : `${v}_${u}`;
                    const relScore = pairRelationshipScores.get(pairKey) || 0;
                    return 240 + Math.min(4, Math.abs(relScore)) * 20;
                }
                ...
                return Math.max(120, Math.min(200, 180 - (affinityCount * 16)));
            })
            .strength(d => {
                if (d.type === 'tension') {
                    // Repulsión física suave y controlada
                    return 0.04;
                }
                if (d.type === 'support') return 0.18;
                ...
                return Math.max(0.06, Math.min(0.22, 0.08 + (affinityCount * 0.04)));
            })
        )
    ...
    simulation.on('tick', () => {
        link.attr('d', d => {
            const uNode = (d.source && typeof d.source === 'object' && d.source.x !== undefined) ? d.source : nodeMap[d.source?.id || d.source];
            const vNode = (d.target && typeof d.target === 'object' && d.target.x !== undefined) ? d.target : nodeMap[d.target?.id || d.target];
            if (!uNode || !vNode || uNode.x === undefined || vNode.x === undefined) return '';
            ...
            const startX = (d.bidirectional && d.type !== 'support' && d.type !== 'tension') ? (sx + (dx / dist) * rSource) : sx;
            const startY = (d.bidirectional && d.type !== 'support' && d.type !== 'tension') ? (sy + (dy / dist) * rSource) : sy;
            const endX = (d.type === 'support' || d.type === 'tension') ? tx : (tx - (dx / dist) * rTarget);
            const endY = (d.type === 'support' || d.type === 'tension') ? ty : (ty - (dy / dist) * rTarget);
...
document.addEventListener('fullscreenchange', () => {
    ...
    if (document.fullscreenElement) {
        ...
        if (hideHudBtn) hideHudBtn.style.display = 'inline-flex';
    } else {
        wrapper.style.padding = '0';
        container.style.height = '520px';
        container.style.borderRadius = '12px';
        if (exitBtn) exitBtn.style.display = 'none';
        if (hideHudBtn) hideHudBtn.style.display = 'none';
        if (restoreHudBtn) restoreHudBtn.style.display = 'none';
        if (hudPanel) hudPanel.classList.remove('hud-minimized');
    }
...
function renderGroupDiagnostic(group) {
    ...
    if (activeTypes.length === 0) {
        ...
        observations.forEach(obs => {
            const mentioned = getMentionedStudents(students, obs.text);
            mentioned.forEach(s => {
